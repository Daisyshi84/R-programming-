...
output:
  ...
  df_print: paged
params:
  Redcap_file_download_date: "2024-09-04_1047" 
---
...
clinical_filename<- "C:\\Users\\mshi27\\Downloads\\NACAttackClinicLevel_DATA_2024-09-04_1048.csv"
```
...
Randomized<-original_data_with_new_race %>%
   ...
   dplyr::select(patient_id,Age,Sex,Race,ec_eye.factor) %>% 
   na.omit()
  
  
eye_level_data <- Randomized %>%
  left_join(Sphere, by = c("patient_id"))%>%
...
Randomized1<-original_data_with_new_race %>%
   dplyr::filter(patient_id %in% r$patient_id)  %>% 
   dplyr::select(patient_id,Age,Sex,Race,rd_eye.factor) %>% 
   na.omit() %>% 
  mutate(Age = as.numeric(Age))
 

eye_level_data1 <- Randomized1 %>%
  left_join(Sphere1, by = c("patient_id"))%>%
...
BCVA_CERT <- certification_redcap %>%
  ...
  mutate(
    ...
    month_to_expire = round(as.numeric(difftime(as.Date(`expiration date`), Sys.Date(), units = "days") / 30), 0),
...
# # Create boxplot with enhanced aesthetics, narrower box width, and no x-axis
# ggplot(CV1, aes(y = drug_taken_percent)) +
#   geom_boxplot(width = 0.1, fill = "skyblue", color = "darkblue", outlier.color = "red", outlier.shape = 16, outlier.size = 2) +
#   scale_y_continuous(labels = percent_format, limits = c(0, 100)) +
#   labs(title = "Boxplot of Drug Taken Percent",
#        y = "Drug Taken Percent") +
#   theme_minimal() +
#   theme(
#     plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
...
# 
# 
# # Create boxplot with enhanced aesthetics, narrower box width, and large x scale
# ggplot(CV1, aes(x = factor(1), y = drug_taken_percent)) +
#   geom_boxplot(width = 0.1, fill = "skyblue", color = "darkblue", outlier.color = "red", outlier.shape = 16, outlier.size = 2) +
#   scale_y_continuous(labels = percent_format, limits = c(0, 100)) +
#   scale_x_discrete(expand = expansion(mult = c(0.1, 0.1))) +  # Add more space around the boxplot
#   labs(title = "Boxplot of Drug Taken Percent",
#        y = "Drug Taken Percent",
#        x = "") +
#   theme_minimal() +
...
C1A_UK <-  read.csv("C:\\Users\\mshi27\\Downloads\\C1A UK dist to sites and receipt_XK2024-06-26.csv")
...
C1B_US <- read.csv("C:\\Users\\mshi27\\Downloads\\C1B US dist to sites and receipt_XK2024-08-09.csv")
C1B_US<-C1B_US %>% mutate(test = ifelse(site==actual.site,"same","no")) %>% 
  mutate(site =  ifelse(test == "no", actual.site, site)) %>% 
  select(-actual.site,-test)

 

C1B_US <- C1B_US %>% setNames(tolower(names(.))) %>% mutate(series = "C1B")  

C1C_NL<- read.csv("C:\\Users\\mshi27\\Downloads\\C1C NL dist to sites and receipt_XK2024-06-26.csv") 
C1C_NL<- C1C_NL %>% setNames(tolower(names(.))) %>% mutate(series = "C1C") 
 
 

ALL<- rbind(C1A_US,C1A_EU,C1A_UK,C1B_US,C1C_NL) %>% 
...
# NAC47_barcode_baseline<- Baseline %>% 
#   dplyr::select(
...
patient_id_discussed_for_outgoing_barcode <- c("1201","3404")
...
withdraw_date <- sample %>% 
  ...
  mutate(withdraw  = paste0("Patient ID : ",patient_id, " Date of withdraw :", ex_withdrawdt)%>% toString()) %>% 
   mutate(site = as.character(site)) %>% 
  select(site, withdraw)
  #make prediction of next rd assignment; 
...
write.csv(plasma_grading_not_analyzed,"plasma_grading_not_analyzed_Sep3.csv")
...
select(patient_id,redcap_event_name.factor,redcap_data_access_group.factor,redcap_repeat_instrument, OD_OS=fxeye,fxfixloss,fxbcea63,fxbcea95,fxdistprl)%>% 
...
eye_level_data_for_color_analysis <- Randomized1 %>%
  left_join(Mean_Sensitivity1, by = c("patient_id")) %>%
  left_join(EZ_width1, by = c("patient_id", "OD_OS", "redcap_event_name.factor")) %>%
...
```{r, message=FALSE, results="asis",echo=FALSE,warning=FALSE}
#  
# patient_id_discussed <- c("1805","2202","2910","2911","1301", "1604","1203", "1604","1402","3215","1306","2004" ,"2714", "2801", "2915", "2917", "2920", "2921", "2923","1217", "1801", "2701",  "2702", "2704", "2705", "2706", "2707","1306", "2714", "2801", "2915", "2917", "2920", "2923", "3105", "1301", "1604", "", "1604", "3401", "", "1217", "1801", "2701", "2702", "2704", "2705", "2706", "2707")
# 
# patient_id_discussed <- unique(patient_id_discussed)
# ###### this is the list of the patient_id_discussed;
# ################################################################################################################
# cat("Patient ID for Daisy to take notes:",
#   PV1 %>%
#     mutate(Notes = ifelse(patient_id %in% patient_id_discussed, 'Discussed', 'New Case')) %>%
#     filter(Category == "Scheduled but not completed, outside window") %>%
#     pull(patient_id) %>%
#     paste0('"', ., '"', collapse = ', '),
# 
#   PV1 %>%
#     mutate(Notes = ifelse(patient_id %in% patient_id_discussed, 'Discussed', 'New Case')) %>%
#     filter(ad_study_meds == 0) %>%
#     pull(patient_id) %>%
#     paste0('"', ., '"', collapse = ', '),
# 
#   window %>%
#     mutate(Notes = ifelse(patient_id %in% patient_id_discussed, 'Discussed', 'New Case')) %>%
#     filter(randomized_window == "***Alert: Passed BSL window & NOT randomized") %>%
#     pull(patient_id) %>%
#     paste0('"', ., '"', collapse = ', '),
# 
#   CV1 %>%
...
#   CV1 %>%
#     mutate(Notes = ifelse(patient_id %in% patient_id_discussed, 'Discussed', 'New Case')) %>%
#     filter(Category == "Completed Within Visit Schedule Window" & is.na(drug_used)) %>%
#     pull(patient_id) %>%
#     paste0('"', ., '"', collapse = ', '),
# 
#   sep = ", "
...
# #########################################################################################
# 
# # Print alert for patients who passed BSL window and were not randomized
# cat("*** Alert: Passed BSL window & NOT randomized ***\n")
# alert_ids <- window %>% 
#   mutate(Notes = ifelse(patient_id %in% patient_id_discussed , 'Discussed', 'New Case')) %>% 
#   filter(randomized_window == "***Alert: Passed BSL window & NOT randomized" ) %>% 
#   mutate(Notes = paste0(patient_id,":",Notes) %>% toString()) %>% 
#   filter(Notes=="New Case") %>% 
#   pull(Notes) %>% unique()%>% kable()
# 
# alert_ids
# cat("\n")
# 
# # Print phone call visit 1 issues
# cat("Phone call Visit 1: Scheduled but not completed, outside window\n")
# scheduled_not_completed_ids <- PV1 %>% 
#   mutate(Notes = ifelse(patient_id %in% patient_id_discussed , 'Discussed', 'New Case')) %>% 
#   filter(Category == "Scheduled but not completed, outside window" ) %>% 
#   mutate(Notes = paste0(patient_id,":",Notes) %>% toString()) %>% 
#   filter(Notes=="New Case") %>% 
...
# 
# 
# print(scheduled_not_completed_ids%>% kable())
# cat("\n")
# 
# 
# cat("Phone call Visit 1: Patient IDs of Patients Who Did Not Take Medication\n")
# not_taking_meds_ids_phone <- PV1 %>% 
#   mutate(Notes = ifelse(patient_id %in% patient_id_discussed , 'Discussed', 'New Case')) %>% 
#   filter(ad_study_meds == 0 ) %>% 
#   mutate(Notes = paste0(patient_id,":",Notes) %>% toString()) %>% 
#   filter(Notes=="New Case") %>% 
#   pull(Notes) %>% unique()
# print(not_taking_meds_ids_phone %>% kable())
# cat("\n")
# 
# # Print in-clinic follow-up visit 1 issues
# cat("In-clinic Follow-up Visit 1: Scheduled but not completed, outside window\n")
# scheduled_not_completed_clinic_ids <- CV1 %>% 
#    mutate(Notes = ifelse(patient_id %in% patient_id_discussed , 'Discussed', 'New Case')) %>% 
#   filter(Category == "Scheduled but not completed, outside window" ) %>% 
#   mutate(Notes = paste0(patient_id,":",Notes) %>% toString()) %>% 
#   filter(Notes=="New Case") %>% 
#   pull(Notes) %>% unique()
# print(scheduled_not_completed_clinic_ids %>% kable())
# cat("\n")
# 
# cat("In-clinic Follow-up Visit 1: Patient IDs of Patients Who Did Not Take Medication\n")
# not_taking_meds_ids_clinic <- CV1 %>% 
#   mutate(Notes = ifelse(patient_id %in% patient_id_discussed , 'Discussed', 'New Case')) %>% 
#   filter(ad_study_meds == 0 ) %>% 
#   mutate(Notes = paste0(patient_id,":",Notes) %>% toString()) %>% 
#   filter(Notes=="New Case") %>% 
#   pull(Notes) %>% unique()
# print(not_taking_meds_ids_clinic %>% kable())
# cat("\n")
# 
# 
# cat("Among M04.5 Completed Within Visit Schedule Window group, below patients did not have NAC47 psmd_visitdt filled to indicate the used or not used drug barcode.\n")
# CV1 %>% 
#   mutate(Notes = ifelse(patient_id %in% patient_id_discussed , 'Discussed', 'New Case'))  %>% 
#   filter(Category=="Completed Within Visit Schedule Window" & is.na(drug_used)) %>% 
#  mutate(Notes = paste0(patient_id,":",Notes) %>% toString()) %>% 
...
cat("drug_taken_percent lower than 85%")
print(drug_taken_percent_discuss %>% kable() )
cat("\n")
